...
export class AppComponent implements OnInit {
  ...
  mapStateToProps() {
    this.store
      .select(AuthSelectors.getLoggedOutPopupActive)
      .pipe(takeUntil(this.unmount$))
      .subscribe((loggedOutPopupActive) => {
  ...
  ngOnInit() {
    if (typeof window.localStorage.authToken !== "undefined") {
      ...
      if (Date.now() > jwtToken.exp * 1000) {
        ...
        this.store.dispatch(
          AuthActions.loginSuccess({ jwtToken: window.localStorage.authToken })
        );
      }
      ...
      if (Date.now() > jwtToken.exp * 1000) {
        ...
        window.localStorage.authToken = window.localStorage[
          "ngStorage-token"
        ].replace(/^\"+|\"+$/g, "");
        this.store.dispatch(
          AuthActions.loginSuccess({ jwtToken: window.localStorage.authToken })
