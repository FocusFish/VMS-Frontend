...
export class AuthEffects {
  ...
  login$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(
        (action: { username: string; password: string; type: string }) => {
          return this.authService.login(action.username, action.password).pipe(
            filter((response: any, index: number) =>
              this.apiErrorHandler(response, index, false)
            ),
            map((auth: any) => {
  ...
  setLogoutCountdown$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        mergeMap((action: any) => {
          ...
          return interval(60000).pipe(
            // Every minute (milliseconds)
            takeUntil(this.logoutTimePassed$),
            withLatestFrom(
              this.store.select(AuthSelectors.getDecodedAuthToken)
            ),
            map(([intervalCount, decodedAuthToken]) => {
              const timeToLogout = Math.round(
                decodedAuthToken.exp - Date.now() / 1000
              );
              if (timeToLogout < 0) {
                // Are we logged out?
                ...
                this.store.dispatch(
                  AuthActions.setTimeToLogout({ timeToLogout: null })
                );
              ...
              } else if (timeToLogout < 3600) {
                // Less then 1 hour (in seconds)
                this.store.dispatch(
                  AuthActions.setTimeToLogout({ timeToLogout })
                );
              }
            })
          );
    ...
    this.actions$.pipe(
      ...
      mergeMap((action: any) => {
        return this.authService
          .getUserContext(action.payload.jwtToken.raw)
          .pipe(
            filter((response: any, index: number) =>
            ...
            map((context: any) => {
              let userContext = context.contextSet.contexts.find(
                (context) => typeof context.scope !== "undefined"
              );

              if (!userContext) {
                userContext = context.contextSet.contexts[0];
              }
              ...
              if (
                ...
                const timezone =
                  userSettingsParsed.timezone ||
                  UserSettingsReducer.initialState.timezone;
                response.push(UserSettingsActions.setTimezone({ timezone }));
                response.push(
                  UserSettingsActions.setExperimentalFeaturesEnabled({
                    experimentalFeaturesEnabled:
                      userSettingsParsed.experimentalFeaturesEnabled,
                  })
                );
              } else {
                response.push(
                  UserSettingsActions.setTimezone({
                    timezone: UserSettingsReducer.initialState.timezone,
                  })
                );
              }

              if (
                typeof mapSettings !== "undefined" &&
                mapSettings.optionValue !== "SYSTEM_DEFAULT_VALUE"
              ) {
                response.push(
                  MapSettingsActions.replaceSettings({
                    settings: JSON.parse(mapSettings.optionValue),
                  })
                );
              }
              if (
                typeof mapLocations !== "undefined" &&
                mapLocations.optionValue !== "SYSTEM_DEFAULT_VALUE"
              ) {
                response.push(
                  MapSettingsActions.setMapLocations({
                    mapLocations: JSON.parse(mapLocations.optionValue),
                  })
                );
              }
              response.push(
                MapActions.setMapSettingsLoaded({ mapSettingsLoaded: true })
              );
            ...
            catchError((err) =>
              of(NotificationsActions.addError(err.toString()))
            )
          );
      })
    )
