...
export class UserSettingsComponent implements OnInit, OnDestroy {
  ...
  public isDataFetched$: Subject<{ map?: boolean; user?: boolean }> =
    new Subject<{ map?: boolean; user?: boolean }>();
  ...
  mapStateToProps() {
    this.isDataFetched$
      .pipe(
        ...
        filter(
          (isSettingsReady) =>
            isSettingsReady.map === true && isSettingsReady.user === true
        )
      )
      .subscribe({
    ...
    this.store
      ...
      .subscribe((mapSettings) => {
        ...
        this.isDataFetched$.next({
          map: Object.entries(mapSettings).length !== 0,
        });
      });
    this.store
      .select(UserSettingsSelectors.getUserSettings)
      .pipe(takeUntil(this.unmount$))
  ...
  mapDispatchToProps() {
    this.save = () => {
      this.store.dispatch(
        MapSettingsActions.saveSettings({
          settings: {
            ...this.mapSettings,
            ...
            forecastsVisible:
              this.formValidator.value.mapSettings.forecastsVisible,
            startZoomLevel:
              this.formValidator.value.mapSettings.mapStartPosition
                .startZoomLevel,
            unitOfDistance: this.formValidator.value.mapSettings.unitOfDistance,
            startPosition: {
              latitude: parseFloat(
                this.formValidator.value.mapSettings.mapStartPosition.latitude
              ),
              longitude: parseFloat(
                this.formValidator.value.mapSettings.mapStartPosition.longitude
              ),
            },
            forecastInterval: parseInt(
              this.formValidator.value.mapSettings.mapLimits.forecastInterval,
              10
            ),
            tracksMinuteCap: parseInt(
              this.formValidator.value.mapSettings.mapLimits.tracksMinuteCap,
              10
            ),
            assetColorMethod:
              this.formValidator.value.mapSettings.assetColorMethod,
      ...
      this.store.dispatch(
        UserSettingsActions.setExperimentalFeaturesEnabled({
          experimentalFeaturesEnabled:
            this.formValidator.value.userSettings.experimentalFeaturesEnabled,
          save: true,
        })
      );
    };
