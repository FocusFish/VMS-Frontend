...
type ExtendedMovement = Readonly<
  AssetTypes.Movement & {
...
export class PositionsComponent implements OnChanges {
  ...
  sortData(sort: Sort) {
    ...
    this.sortedPositions = positions.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case "oceanRegion":
          return compareTableSortString(a.oceanRegion, b.oceanRegion, isAsc);
        case "status":
          return compareTableSortString(a.status, b.status, isAsc);
        case "source":
          return compareTableSortString(a.source, b.source, isAsc);
        default:
          return 0;
      }
    });
  ...
  exportPositionsToCSV() {
    ...
    csv =
      csv +
      positionsForCSV.reduce((acc, pos, mtIndex) => {
        return (
          acc +
          this.displayedColumns.reduce((csvRow, column, index) => {
            return (
    ...
    const exportedFilenmae =
      "lastPositions." + moment().format("YYYY-MM-DD.HH_mm") + ".csv";
