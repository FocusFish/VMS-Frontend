...
export class ListForAssetComponent implements OnChanges {
  ...
  @Input() changeCurrentMobileTerminal: (
  ...
  ngOnChanges() {
    this.formattedMobileTerminals = this.mobileTerminals.map(
      (mobileTerminal: MobileTerminalTypes.MobileTerminal) => ({
        ...mobileTerminal,
        ...
        channels: mobileTerminal.channels
          .slice()
          .sort(
            (
              c1: MobileTerminalTypes.Channel,
              c2: MobileTerminalTypes.Channel
            ) => {
