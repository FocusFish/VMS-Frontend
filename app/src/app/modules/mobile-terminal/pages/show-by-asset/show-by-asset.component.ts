  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  mapStateToProps() {
    this.store
      .select(MobileTerminalSelectors.getMobileTerminalsForUrlAsset)
      .pipe(takeUntil(this.unmount$))
      .subscribe((mobileTerminals) => {
        this.mobileTerminals = mobileTerminals;
        if (this.mobileTerminals.length > 0) {
          if (
            typeof this.currentMobileTerminal === "undefined" ||
            !this.mobileTerminals.some(
              (mobileTerminal) =>
                mobileTerminal.id === this.currentMobileTerminal.id
            )
          ) {
            this.currentMobileTerminal = this.mobileTerminals[0];
          }
          this.activeMobileTerminal = this.mobileTerminals.find(
            (mobileTerminal) => mobileTerminal.active
          );

          if (this.mobileTerminalHistoryNotUpToDate) {
            this.mobileTerminalHistoryNotUpToDate = false;
            this.store.dispatch(
              MobileTerminalActions.getMobileTerminalHistoryForAsset({
    ...
    this.store
      .select(
        MobileTerminalSelectors.getMobileTerminalHistoryFilteredForUrlAsset
      )
      .pipe(takeUntil(this.unmount$))
      .subscribe(
        (
          mobileTerminalHistoryList: MobileTerminalTypes.MobileTerminalHistoryList
        ) => {
  ...
  mapDispatchToProps() {
    this.saveMobileTerminal = (
      ...
      this.store.dispatch(
        MobileTerminalActions.saveMobileTerminal({ mobileTerminal })
      );
    };
  ...
  mapFunctionsToProps() {
    this.changeCurrentMobileTerminal = (
      mobileTerminal: MobileTerminalTypes.MobileTerminal
    ) => {
  ...
  activeToggleDisabled() {
    const disabled =
      typeof this.activeMobileTerminal !== "undefined" &&
