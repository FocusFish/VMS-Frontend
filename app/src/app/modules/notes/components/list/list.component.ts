...
export class ListComponent implements OnChanges {
  ...
  ngOnChanges() {
    ...
    this.filteredNotes = this.formattedNotes
      .filter(
        (note) =>
          note.searchableString.indexOf(this.searchString.toLowerCase()) !== -1
      )
      .map((note) => note.note);
  }
  ...
  searchNotes(searchString: string) {
    this.filteredNotes = this.formattedNotes
      .filter(
        (note) =>
          note.searchableString.indexOf(searchString.toLowerCase()) !== -1
      )
      .map((note) => note.note);
  }
  ...
  public cancelEditNote = (noteId: string) => () => {
    if (this.notesInEditMode.includes(noteId)) {
      this.notesInEditMode = this.notesInEditMode.filter(
        (iNoteId) => iNoteId !== noteId
      );
    }
  };
