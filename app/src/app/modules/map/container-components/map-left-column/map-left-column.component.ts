...
import {
  MapSavedFiltersActions,
  MapSavedFiltersTypes,
  MapSavedFiltersSelectors,
...
export class MapLeftColumnComponent implements OnInit, OnDestroy {
  ...
  public setActiveInformationPanel: (
    activeInformationPanel: string | null
  ) => void;

  public filtersActive: Readonly<{
    readonly [filterTypeName: string]: boolean;
  }>;
  public setGivenFilterActive: (
    filterTypeName: string,
    status: boolean
  ) => void;
  public filterAssets: (
    filterQuery: ReadonlyArray<AssetTypes.AssetFilterQuery>
  ) => void;

  public assetsForAssetGroups: Readonly<{
    readonly [assetId: string]: AssetTypes.Asset;
  }>;
  public currentFilterQuery$: Observable<
    ReadonlyArray<AssetTypes.AssetFilterQuery>
  >;
  public saveFilter: (filter: MapSavedFiltersTypes.SavedFilter) => void;
  public deleteFilter: (filterId: string) => void;
  public savedFilters$: Observable<
    ReadonlyArray<MapSavedFiltersTypes.SavedFilter>
  >;
  ...
  // Curried functions
  public setGivenFilterActiveCurry =
    (filterTypeName: string) => (status: boolean) =>
      this.setGivenFilterActive(filterTypeName, status);
  public toggleLastInPanelTree =
    (panelTree: ReadonlyArray<string>) => (status: boolean) => {
      if (status) {
  ...
  public setActivePanelAndShowColumn = (
  ...
  public createIncidentNote: (
    incidentId: number,
    note: NotesTypes.NoteParameters
  ) => void;

  public createNoteCurried = (note: string) => {
    return this.createIncidentNote(this.selectedIncident.id, {
      note,
  ...
  mapStateToProps() {
    this.store
      .select(MapSelectors.getActiveLeftPanel)
      .pipe(takeUntil(this.unmount$))
      .subscribe((activePanel) => {
        this.activePanel = activePanel;
      });
    this.store
      .select(MapSelectors.getFiltersActive)
      .pipe(takeUntil(this.unmount$))
      .subscribe((filtersActive) => {
        this.filtersActive = filtersActive;
      });
    this.currentFilterQuery$ = this.store.select(
      AssetSelectors.selectFilterQuery
    );
    this.savedFilters$ = this.store.select(MapSavedFiltersSelectors.getFilters);
    this.activeFilters$ = this.store.select(
      MapSavedFiltersSelectors.selectActiveFilters
    );
    this.store
      .select(MapSavedFiltersSelectors.getAssetGroupFilters)
      .pipe(takeUntil(this.unmount$))
      .subscribe((assetGroupFilters) => {
        this.assetGroupFilters = assetGroupFilters;
        const assetIds = [
          ...new Set(
            assetGroupFilters.reduce(
              (acc: ReadonlyArray<string>, assetGroupFilter) => {
                const filter = assetGroupFilter.filter.find(
                  (f) => f.type === "GUID"
                );
    ...
    this.searchAutocompleteResult$ = this.store.select(
      AssetSelectors.getSearchAutocomplete
    );
    this.store
      .select(IncidentSelectors.getIncidentsByTypeAndStatus)
      .pipe(takeUntil(this.unmount$))
      .subscribe(
    ...
    this.store
      .select(AssetSelectors.extendedDataForSelectedAssets)
      .pipe(takeUntil(this.unmount$))
      .subscribe((selectedAssets) => {
        this.selectedAsset = selectedAssets.find(
          (selectedAsset) => selectedAsset.currentlyShowing
        );
        if (
          ...
          this.store.dispatch(
            MapActions.setActiveLeftPanel({ activeLeftPanel: ["filters"] })
          );
          this.expandColumn(false);
        }
      });
    this.activityTracks$ = this.store.select(
      ActivitySelectors.getActivityTracks
    );
    this.store
      .select(IncidentSelectors.getSelectedIncident)
      .pipe(takeUntil(this.unmount$))
      .subscribe((incident) => {
        this.selectedIncident = incident;
      });
    this.store
      .select(AssetSelectors.getAssetsForAssetGroups)
      .pipe(takeUntil(this.unmount$))
      .subscribe((assets) => {
        this.assetsForAssetGroups = assets;
      });
    this.store
      .select(MapSettingsSelectors.getMapSettings)
      .pipe(takeUntil(this.unmount$))
  ...
  mapDispatchToProps() {
    ...
    this.setActiveInformationPanel = (
      activeInformationPanel: string | null
    ) => {
      if (this.mapSettings.autoHelp === true) {
        this.store.dispatch(
          MapActions.setActiveInformationPanel({ activeInformationPanel })
        );
      }
    };
    this.setGivenFilterActive = (filterTypeName: string, status: boolean) =>
      this.store.dispatch(
        MapActions.setGivenFilterActive({ filterTypeName, status })
      );
    this.filterAssets = (
      filterQuery: ReadonlyArray<AssetTypes.AssetFilterQuery>
    ...
    this.deactivateSavedFilter = (filterId: string) =>
      this.store.dispatch(
        MapSavedFiltersActions.deactivateFilter({ filterId })
      );
    ...
    this.selectAsset = (assetId: string) => {
      ...
      this.store.dispatch(
        AssetActions.getLastPositionsForSelectedAsset({ assetId })
      );
    };
    ...
    this.clearNotificationsForIncident = (incident: IncidentTypes.Incident) =>
      this.store.dispatch(
        IncidentActions.clearNotificationsForIncident({ incident })
      );
    this.clearSelectedIncident = () =>
      this.store.dispatch(IncidentActions.clearSelectedIncident());
    this.createIncidentNote = (
      incidentId: number,
      note: NotesTypes.NoteParameters
    ...
    this.updateIncidentType = (
      ...
      this.store.dispatch(
        IncidentActions.updateIncidentType({
          incidentId,
          incidentType,
          expiryDate,
        })
    ...
    this.updateIncidentStatus = (
      ...
      this.store.dispatch(
        IncidentActions.updateIncidentStatus({ incidentId, status, expiryDate })
      );
    this.updateIncidentExpiry = (incidentId: number, expiryDate: number) =>
      this.store.dispatch(
        IncidentActions.updateIncidentExpiry({ incidentId, expiryDate })
      );
  }
    ...
    this.countNotificationsOfType = (
      ...
      if (typeof incidentNotifications !== "undefined") {
        return Object.values(incidentNotifications).reduce(
          (
            acc: number,
            incidentNotification: IncidentTypes.IncidentNotifications
          ) => {
  ...
  openIncidentTypeFormDialog(type: string) {
    ...
    dialogRef
      .afterClosed()
      .pipe(first())
      .subscribe((detachResult) => {
        if (typeof detachResult !== "undefined" && detachResult !== false) {
          this.updateIncidentType(
            this.selectedIncident.id,
            detachResult.type,
            detachResult.expiryDate
          );
          this.createIncidentNote(this.selectedIncident.id, {
            note: detachResult.note,
            assetId: this.selectedAsset.asset.id,
          });
        }
      });
  ...
  private getHTMLElementFromEvent(event: CdkDragDrop<string[]>) {
    return event.container.element.nativeElement.firstElementChild
