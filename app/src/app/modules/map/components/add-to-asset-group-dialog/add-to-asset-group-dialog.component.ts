...
export class AddToAssetGroupDialogComponent {
  ...
  private readonly assetGroupFilters: Readonly<{
  ...
  constructor(
    ...
    this.assetGroupFilters = this.data.assetGroupFilters.reduce(
      (acc, assetGroupFilter) => {
  ...
  returnUpdatedGroup() {
    if (this.selectedGroup === "new") {
      ...
      typeof this.assetGroupFilters[this.selectedGroup] !== "undefined"
    ) {
      return {
        ...this.assetGroupFilters[this.selectedGroup],
        filter: this.assetGroupFilters[this.selectedGroup].filter.map(
          (filter: MapSavedFiltersTypes.AssetFilterQuery) => {
            if (filter.type !== "GUID") {
              return filter;
            } else {
              return {
                ...
                values: [
                  ...filter.values,
                  ...this.data.selectedAssets.map((asset) => asset.asset.id),
                ],
              };
            }
          }
