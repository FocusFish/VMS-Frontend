...
export class EditAssetGroupDialogComponent {
  ...
  constructor(
    ...
    this.assetGroupFilterQuery = data.assetGroupFilter.filter.find(
      (filterQuery) => filterQuery.type === "GUID"
    );
    this.miniAssets = this.assetGroupFilterQuery.values.map((assetId) => ({
      assetId,
      assetName: data.assets[assetId] ? data.assets[assetId].name : null,
    }));
    this.filterName = new UntypedFormControl(
      data.assetGroupFilter.name,
      Validators.required
    );
  }
  ...
  result() {
    const ids = this.assetGroupFilterQuery.values.filter(
      (assetId) => !this.assetsToRemove.includes(assetId)
    );
    if (
      this.filterName.value !== this.data.assetGroupFilter.name ||
      this.assetGroupFilterQuery.values.length !== ids.length
    ) {
      return {
