...
export class IncidentComponent implements OnChanges {
  ...
  @Input() createManualMovement: (
    manualMovement: AssetTypes.ManualMovement
  ) => void;
  @Input() updateIncidentType: (
    incindentId: number,
    incidentType: IncidentTypes.IncidentTypes,
    expiryDate?: number
  ) => void;
  @Input() updateIncidentStatus: (
    incindentId: number,
    status: string,
    expiryDate?: number
  ) => void;
  @Input() updateIncidentExpiry: (
    incindentId: number,
    expiryDate: number
  ) => void;
  @Input() createNote: (
  ...
  public lastKnownPositionFormatted: Readonly<{
    latitude: string;
    longitude: string;
  }>;
  ...
  public createNoteCurried = (note: string) => {
    return this.createNote(this.incident.id, {
      note,
      assetId: this.asset.asset.id,
    });
  };
  ...
  public checkPermissionForRegisterAttemptedContact() {
    ...
    return (
      typeof permission[this.incident.type] !== "undefined" &&
  ...
  openIncidentAttemptedContactDialog() {
    ...
    dialogRef
      ...
      .subscribe((detachResult) => {
        if (typeof detachResult !== "undefined" && detachResult !== false) {
          this.changeStatus(
            IncidentTypes.AssetNotSendingStatuses.ATTEMPTED_CONTACT
          );
          this.createNoteCurried(detachResult.note);
        }
      });
