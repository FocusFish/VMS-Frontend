...
describe("AssetForecastComponent", () => {
  ...
  it("should draw forecast correctly", () => {
    ...
    component["vectorSource"] = {
      ...
      addFeature: (feature) => {
        savedFeatures[feature.getId()] = feature;
      },
    };
    ...
    const calculatedCord2LonLat = toLonLat(
      savedFeatures[id].getGeometry().getCoordinates()
    );
    const cord2LonLat = toLonLat(coordinates2);
    expect(calculatedCord2LonLat[0].toFixed(3)).toEqual(
      cord2LonLat[0].toFixed(3)
    );
    expect(calculatedCord2LonLat[1].toFixed(4)).toEqual(
      cord2LonLat[1].toFixed(4)
    );
    ...
    const returningPosition = toLonLat(
      savedFeatures[id].getGeometry().getCoordinates()
    );
    expect(returningPosition[0].toFixed(3)).toEqual(
      AssetMovementStub.movement.location.longitude.toFixed(3)
    );
    expect(returningPosition[1].toFixed(4)).toEqual(
      AssetMovementStub.movement.location.latitude.toFixed(4)
    );

    component.forecastInterval = null;
    component.drawFuturePosition(AssetMovementStub);
    const coordinatesWithDefaultInterval = savedFeatures[
      "futurePos_0_" + AssetMovementStub.asset
    ]
      .getGeometry()
      .getCoordinates();
    expect(coordinatesWithDefaultInterval).toEqual([
      2575618.876426982, 9624472.214963332,
    ]);
  });
    ...
    component["vectorSource"] = {
      getFeatureById: (featureId) => {
        const feature = features.filter(
          (value, index, arr) => value.getId() === featureId
        );
        if (feature.length > 0) {
          return feature[0];
        } else {
      ...
      addFeature: (feature) => {
        if (
          component["vectorSource"].getFeatureById(feature.getId()) === null
        ) {
          features.push(feature);
        }
      },
