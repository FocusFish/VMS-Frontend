...
} from "@app/helpers/wgs84-formatter";
...
export class AssetSearchComponent implements OnChanges {
  ...
  @Input() setActiveInformationPanel: (
    activeInformationPanel: string | null
  ) => void;
  @Input() setActiveRightPanel: (
  ...
  searchKeyUp = (event) => {
    if (
      ...
      this.searchQuery.match(
        /^(\/c)\s*(N|S)\s+(\d{1,3})(°)\s+(\d{1,2})(\.)(\d{1,})(')\s+(W|E)\s+(\d{1,3})(°)\s+(\d{1,2})(\.)(\d{1,})(')$/
      )
    ) {
      ...
      event.key === "Enter" &&
      this.searchQuery.match(
        /^(\/c)\s*(\d{1,3})\s+(\d{1,2})[,.]{1}(\d+)\s+(\d{1,3})\s+(\d{1,2})[,.]{1}(\d+)/
      )
    ) {
      ...
      const location = convertDDMToDDJustNumbers(
        lat,
        latitudeMS,
        long,
        longitudeMS
      ...
      event.key === "Enter" &&
      this.searchQuery.match(/^(\/c)\s*(\-?\d+(\.\d+)?),?\s*(\-?\d+(\.\d+)?)$/)
    ) {
      const searchQueryParts = this.searchQuery.split(
        /^(\/c)\s*(\-?\d+(\.\d+)?),?\s*(\-?\d+(\.\d+)?)$/
      );
  ...
  optionSelected = (event) => {
    ...
    const selectedAsset = this.autocompleteResult.find(
      (asset) => asset.asset.id === selectedId
    );
    this.selectAsset(selectedAsset.asset.id);
    ...
    setTimeout(() => document.getElementById("asset-search-field").blur(), 0);
  };
  ...
  ngOnChanges() {
    if (typeof this.autocompleteResult !== "undefined") {
      if (this.autocompleteResult.length === 0) {
        ...
        let lastIndex = 0;
        this.searchResults = this.autocompleteResult
          .sort((a, b) => {
          ...
          .reduce((acc, assetWrapper, index) => {
            ...
            if (
              typeof acc[index] === "undefined" ||
              acc[index].id !== assetWrapper.asset.id
            ) {
              let setIrcs = !assetWrapper.asset.ircs
                ? ""
                : " --- " + assetWrapper.asset.ircs;
              acc[index] = {
                id: assetWrapper.asset.id,
                value: assetWrapper.asset.name + setIrcs,
              };
            }
            lastIndex = index;
            return acc;
