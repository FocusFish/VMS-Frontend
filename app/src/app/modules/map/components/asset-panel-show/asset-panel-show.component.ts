...
export class AssetPanelShowComponent {
  ...
  formatLocation(location: Position) {
    const formattedPosition = convertDDToDDM(
      location.latitude,
      location.longitude,
      4,
    );
  ...
  translateOceanRegion(sourceSatelliteId: string) {
    return (
      AssetTypes.OceanRegionTranslation[sourceSatelliteId] || sourceSatelliteId
    );
  }

  getAisClass() {
    if (
      typeof this.selectedAssetsLastPositions.ais.status !== "undefined" &&
      this.selectedAssetsLastPositions.ais.status > ""
    ) {
