...
export class TracksPanelComponent implements OnChanges {
  ...
  @Input() activityTracks: {
    [assetId: string]: ReadonlyArray<ActivityTypes.Activity>;
  };
  ...
  ngOnChanges() {
    ...
    if (
      typeof this.track === "undefined" ||
      typeof this.track.tracks === "undefined"
    ) {
      this.formattedTrack = [];
    } else {
      this.formattedTrack = this.track.tracks.map((position) => ({
        ...
        formattedSpeed:
          typeof position.speed === "number" ? position.speed.toFixed(2) : "",
        heading: position.heading,
        formattedCalculatedSpeed:
          typeof position.calculatedSpeed === "number"
            ? position.speed.toFixed(2)
            : "",
        oceanRegion:
          AssetTypes.OceanRegionTranslation[position.sourceSatelliteId],
        type: position.source === "AIS" ? position.source : "VMS",
        movement: position,
      }));
      if (typeof this.activityTracks[this.track.assetId] !== "undefined") {
        const activities: ReadonlyArray<ExtendedTrack> = this.activityTracks[
          this.track.assetId
        ].reduce((acc, activity) => {
          if (activity.relatedActivities.length > 0) {
            acc = acc.concat(
              activity.relatedActivities.map((relatedActivity) => ({
                ...
                formattedTimestamp: formatUnixtimeSeconds(
                  relatedActivity.occurence
                ),
                latitude: relatedActivity.latitude,
                longitude: relatedActivity.longitude,
                locationDDM: convertDDToDDM(
                  relatedActivity.latitude,
                  relatedActivity.longitude,
                  2
                ),
                speed: null,
                formattedSpeed: "",
                heading: null,
                type: relatedActivity.activityType
                  .toLowerCase()
                  .replace(/^_*(.)|_+(.)/g, (s, c, d) =>
      ...
      this.hasVms = this.formattedTrack.some(
        (position) => position.type === "VMS"
      );
      this.hasAis = this.formattedTrack.some(
        (position) => position.type === "AIS"
      );
      this.hasActivities = this.formattedTrack.some(
        (position) => position.type !== "VMS" && position.type !== "AIS"
      );
  ...
  sortAndFilterData(sort: Sort) {
    ...
    if (!sort.active || sort.direction === "") {
      this.sortedTrack = new MatTableDataSource<ExtendedTrack>(
        filteredPositions
      );
    ...
    this.sortedTrack = new MatTableDataSource<ExtendedTrack>(
      filteredPositions.sort((a, b) => {
        ...
        switch (sort.active) {
          case "timestamp":
            return compareTableSortNumber(a.timestamp, b.timestamp, isAsc);
          case "latitude":
            return compareTableSortNumber(a.latitude, b.latitude, isAsc);
          case "longitude":
            return compareTableSortNumber(a.longitude, b.longitude, isAsc);
          case "speed":
            return compareTableSortNumber(a.speed, b.speed, isAsc);
          case "heading":
            return compareTableSortNumber(a.heading, b.heading, isAsc);
          case "type":
            return compareTableSortString(a.type, b.type, isAsc);
          default:
            return 0;
        }
      })
