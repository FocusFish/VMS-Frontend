...
export class AssetPositionsComponent implements OnInit, OnChanges {
  ...
  public ngOnInit() {
    this.getLastFullPositionsForAsset(
      this.asset.id,
      20,
      this.sourcesToExclude,
      true,
    );
  }
  ...
  public ngOnChanges() {
    if (typeof this.positions === "undefined") {
      ...
      this.extendedPositions = this.positions
        .map((position) => ({
          ...position,
  ...
  public trackByPositionId = (index: number, position: AssetTypes.Movement) => {
    return position;
  };
